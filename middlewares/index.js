...
const checkRateLimitMiddleware = async (req, res, next) => {
  ...
  if (providedAuthKey !== undefined) {
    findUserWithAuthKey(providedAuthKey)
      .then((result) => {
        ...
        if (result.requests >= result.quota) {
          ...
          res.set(
            "Retry-After",
            "24 hours, or contact the API devleoper to expand your Quota."
          );
          res.sendStatus(429);
          res.end();
        } else {
...
const updateRequestsMiddleware = async (req, res, next) => {
  ...
  updateUserRequests(providedAuthKey)
    .then((result) => {
      next();
    })
    .catch((err) => {
    ...
    try {
      ...
      if (result === null || authKey !== result.authKey) {
        res
          ...
          .json({
            message:
              "Your'authentication Key is invalid, Get new key by registration. /n It may be you did put the wrong key in the header when you made the request.",
          });
        res.end();
      } else {
...
// todo exlode this MW to run registrations and random requests
